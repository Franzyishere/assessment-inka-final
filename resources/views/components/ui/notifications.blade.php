...
@if ($notifications->isNotEmpty())
    <div
        class="pointer-events-none fixed inset-x-4 top-5 z-[100000] flex flex-col items-center gap-2.5 sm:inset-x-0 sm:top-6 sm:mx-auto sm:w-full sm:max-w-md"
        aria-live="polite"
        aria-atomic="true"
    >
        @foreach ($notifications as $notification)
            ...
            <div
                ...
                x-init="timer = setTimeout(() => open = false, {{ $isError ? 8500 : 6500 }})"
                x-show="open"
                ...
                class="pointer-events-auto relative w-full overflow-hidden rounded-xl border border-gray-200 bg-white p-4 shadow-[0_16px_40px_-14px_rgba(16,24,40,0.3)]"
            >
                <div class="absolute inset-y-0 left-0 w-1.5 {{ $accent }}"></div>
                <div class="flex items-start gap-3 pl-1">
                    <div class="flex size-10 shrink-0 items-center justify-center rounded-full {{ $iconBox }}">
                        @if ($isSuccess)
                    ...
                    <div class="min-w-0 flex-1 pt-0.5">
                        ...
                        <p class="mt-1 text-sm font-medium leading-5 text-gray-600">{{ $notification['message'] }}</p>
                ...
                <div class="absolute inset-x-0 bottom-0 h-1 bg-gray-100"><div class="h-full {{ $accent }} motion-safe:animate-[toast-progress_6.5s_linear_forwards]"></div></div>
            </div>
        @endforeach
    </div>
@endif
...
>
    <template x-for="toast in toasts" :key="toast.id">
        <div x-transition:enter="transition ease-out duration-300" x-transition:enter-start="-translate-y-5 scale-95 opacity-0" x-transition:enter-end="translate-y-0 scale-100 opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-end="-translate-y-3 scale-95 opacity-0" class="pointer-events-auto relative w-full overflow-hidden rounded-xl border border-gray-200 bg-white p-4 shadow-[0_16px_40px_-14px_rgba(16,24,40,0.3)]">
            <div class="absolute inset-y-0 left-0 w-1.5" :class="toast.type === 'error' ? 'bg-error-500' : (toast.type === 'success' ? 'bg-success-500' : (toast.type === 'warning' ? 'bg-warning-500' : 'bg-brand-500'))"></div>
            <div class="flex items-start gap-3 pl-1">
                <div class="flex size-10 shrink-0 items-center justify-center rounded-full" :class="toast.type === 'error' ? 'bg-error-50 text-error-600' : (toast.type === 'success' ? 'bg-success-50 text-success-600' : (toast.type === 'warning' ? 'bg-warning-50 text-warning-600' : 'bg-brand-50 text-brand-600'))">
                    <svg x-show="toast.type === 'success'" class="size-5" viewBox="0 0 20 20" fill="none"><path d="m5 10 3 3 7-7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    <svg x-show="toast.type !== 'success'" class="size-5" viewBox="0 0 20 20" fill="none"><path d="M10 6v4m0 4h.01M17.5 10a7.5 7.5 0 1 1-15 0 7.5 7.5 0 0 1 15 0Z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
                </div>
                <div class="min-w-0 flex-1"><p class="text-sm font-bold text-gray-900" x-text="toast.title"></p><p class="mt-1 text-sm font-medium leading-5 text-gray-600" x-text="toast.message"></p></div>
            ...
            <div class="absolute inset-x-0 bottom-0 h-1 bg-gray-100"><div class="h-full motion-safe:animate-[toast-progress_5s_linear_forwards]" :class="toast.type === 'error' ? 'bg-error-500' : (toast.type === 'success' ? 'bg-success-500' : (toast.type === 'warning' ? 'bg-warning-500' : 'bg-brand-500'))"></div></div>
        </div>
    </template>
</div>
