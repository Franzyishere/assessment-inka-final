...
<div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
    @forelse($simulations as $simulation)
        <article class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex items-start justify-between gap-4"><div><span class="text-xs font-medium uppercase text-brand-500">Simulasi {{ $simulation->scenario->type->sequence }}</span><h2 class="mt-1 font-semibold text-gray-800 dark:text-white/90">{{ $simulation->scenario->simulationThreePackageLabel() ?? $simulation->scenario->type->name }}</h2><p class="mt-1 text-sm text-gray-500">{{ $simulation->program->name }}</p></div><span class="rounded-full bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">{{ ucfirst($simulation->status) }}</span></div>
