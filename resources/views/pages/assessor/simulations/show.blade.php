...
<section x-data="{ search: '' }" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs">
    ...
    <div class="border-b border-gray-200 bg-gray-50/60 px-5 py-3"><input type="search" x-model.debounce.200ms="search" placeholder="Cari nama atau email peserta..." class="h-9 w-full rounded-md border border-gray-300 bg-white px-3 text-sm sm:max-w-sm"></div>
            ...
            <tbody class="divide-y divide-gray-100">
                @if($isObservation)
                    @forelse($programSimulation->program->participants as $participant)
                        ...
                        @endphp
                        <tr x-show="!search || @js(mb_strtolower($participant->user->name.' '.$participant->user->email)).includes(search.toLowerCase())" class="transition hover:bg-gray-50/80">
                ...
                @else
                    @forelse($programSimulation->sessions as $session)
                        ...
                        @endphp
                        <tr x-show="!search || @js(mb_strtolower($session->participant->user->name.' '.$session->participant->user->email)).includes(search.toLowerCase())" class="transition hover:bg-gray-50/80">
...
<div class="mt-5"><a href="{{ route('asesor.simulations.program', $programSimulation->program) }}" class="crud-btn-secondary"><span aria-hidden="true">←</span> Kembali</a></div>
@endsection
