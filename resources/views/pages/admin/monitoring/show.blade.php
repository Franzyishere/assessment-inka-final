            ...
            @forelse($program->participants as $participant)
                <tr><td class="sticky left-0 bg-white px-5 py-4 dark:bg-gray-900"><div class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $participant->user->name }}</div><div class="text-xs text-gray-500">{{ $participant->user->email }}</div></td>
                    @foreach($program->simulations as $simulation)
                        ...
                        @endphp
                        <td class="px-5 py-4 align-top">@if($notEligible)<span class="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-400">Tidak sesuai kategori</span>@else<span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $styles }}">{{ $labels[$status] ?? ucfirst($status) }}</span>@if($session)<div class="mt-2 space-y-1 text-xs text-gray-500"><p>Mulai: {{ $session->started_at?->format('d M H:i') ?? '-' }}</p><p>Selesai: {{ $session->submitted_at?->format('d M H:i') ?? '-' }}</p>@if($session->events->isNotEmpty())<p class="font-medium text-error-600" title="{{ $session->events->groupBy('event_type')->map->count()->map(fn($count, $type) => str_replace('_', ' ', $type).': '.$count)->implode(', ') }}">{{ $session->events->count() }} aktivitas tercatat</p>@endif @if($status === 'submitted')<p class="font-medium {{ $reviewed ? 'text-success-600' : 'text-warning-600' }}">{{ $reviewed ? 'Penilaian final' : 'Menunggu penilaian' }}</p>@endif</div>@endif @endif</td>
                    @endforeach
                </tr>
