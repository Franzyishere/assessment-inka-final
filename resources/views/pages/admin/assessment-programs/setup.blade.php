...
<form method="POST" action="{{ route('admin.assessment-programs.setup.update', $program) }}"
    ...
    <div class="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
        @if(collect(array_keys(\App\Models\SimulationScenario::SIMULATION_THREE_PACKAGES))->diff(array_keys($simulationThreeOptions))->isEmpty())
            Simulasi 3: admin menetapkan salah satu materi untuk tiap peserta—CI panjang, CI pendek, atau In-Tray. Ketiganya dapat dipilih untuk semua level assessment; asesor hanya melihat hasil penetapan.
        @else
            Program ini tetap menggunakan paket lama karena Simulasi 3 sudah pernah dimulai. Materi dan riwayat pengerjaan dipertahankan.
        @endif
    </div>
